...
$E_f(\rho,S) = \rho(1-S)$ over the $(\rho,S)$ plane, illustrating how high density and low entropy
combine to yield large $E_f$.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.65\textwidth]{efc_Ef_rho_S_heatmap.png}
  \caption{Schematic heatmap of the energy-flow potential
  $E_f(\rho,S) = \rho(1-S)$ as a function of mass density $\rho$ and dimensionless entropy $S$.}
  \label{fig:Ef-map}
\end{figure}

\subsection{Halo profiles: mass and entropy}

EFC-S models halos as joint profiles in mass density and entropy. A simple schematic example is shown
in Figures~\ref{fig:halo-rho} and~\ref{fig:halo-S}, which visualise $\rho_h(r)$ and $S_h(r)$ as
functions of radius $r$.
...
Using the definition of $E_f$ in Eq.~\eqref{eq:D0-Ef-def} and applying the product rule, one obtains
\begin{equation}
  \frac{dE_f}{dt}
    = (1 - S)\,\frac{d\rho}{dt}
    - \rho\,\frac{dS}{dt}.
  \label{eq:D1-expanded}
\end{equation}
...
\begin{equation}
  \nabla E_f(\bm{x})
    = (1 - S(\bm{x}))\,\nabla \rho(\bm{x})
    - \rho(\bm{x})\,\nabla S(\bm{x}),
  \label{eq:D2-gradient}
\end{equation}
which follows directly from Eq.~\eqref{eq:D0-Ef-def} via the product rule.
...
\begin{itemize}
  ...
  \item $\beta$ scales a dissipation term $D_I$ (for example diffusion, noise or degradation of
        structure).
\end{itemize}
Equation~\eqref{eq:C2-coupling} is a minimal base equation that later cognitive layers can extend.
