...
The repository serves as a **machine-readable knowledge graph** compliant with [Schema.org](https://schema.org) standards.

### Core nodes
- **AuthNode (Who):** establishes authorship and provenance.  
- **ConceptNode (What):** defines the scientific core (energy flow, entropy, structure).  
- **MethodologyNode (How):** documents the open reflective process linking human and AI reasoning.

All nodes are unified in [`/schema/site-graph.json`](schema/site-graph.json),  
which functions as the canonical graph root for search engines, LLMs, and metadata crawlers.

---

## 🔹 Open-Science Principles

EFC follows four methodological commitments:

1. **Transparency** — all concepts and reasoning are published openly.  
2. **Reproducibility** — every definition is semantically defined and version-controlled.  
3. **Interoperability** — integrates ORCID, Figshare, GitHub, and WordPress schema.  
4. **Reflectivity** — includes cognitive and AI-assisted reasoning as part of the scientific record.
